const NodeTable = ({ nodes = [] }) => {
  const formatStorage = (bytes) => `${Math.round((bytes ?? 0) / 1024 ** 3)}GB`;
      ...
      <table className="min-w-full divide-y divide-gray-700">
        ...
        <tbody className="divide-y divide-gray-700">
          {nodes.length === 0 ? (
            ...
            nodes.map((pod) => (
              <tr key={pod.pubkey || pod.address} className="hover:bg-gray-700">
                <td className="px-4 py-2">{pod.address ?? "-"}</td>
                ...
                <td className="px-4 py-2 text-center">{pod.version ?? "-"}</td>

                <td className="px-4 py-2 text-right">{pod.credits ?? 0}</td>
              </tr>
            ))
          )}
        </tbody>
